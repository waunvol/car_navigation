...
double ReedsSheppStateSpace::distance(double from[3], double to[3])
{
    return rho_ * reedsShepp(from, to).length();
}

ReedsSheppStateSpace::ReedsSheppPath ReedsSheppStateSpace::reedsShepp(double from[3], double to[3])
{
    double dx = to[0] - from[0], dy = to[1] - from[1], dth = to[2] - from[2];
    double c = cos(from[2]), s = sin(from[2]);
    double x = c*dx + s*dy, y = -s*dx + c*dy;

    return ::reedsShepp(x/rho_, y/rho_, dth);
}

std::vector<int>ReedsSheppStateSpace::CurveType(double from[3], double to[3])
{
    ReedsSheppPath path = reedsShepp(from, to);
...
std::vector<std::vector<float> > ReedsSheppStateSpace::GetCurve(double from[], double to[], double step_size)
{
    ReedsSheppPath path = reedsShepp(from, to);
    ...
    for (double seg=0.0; seg<=dist; seg+=step_size){
        double p_next[3] = {};
        interpolate(from, path, seg/rho_, p_next);
        std::vector<float>temp;
        for(int i=0;i<3;i++)
        {
            temp.push_back(p_next[i]);
        }
...
void ReedsSheppStateSpace::interpolate(double from[3], ReedsSheppPath &path, double seg, double p_next[3])
{
    ...
    for (unsigned int i=0; i<5 && seg>0; ++i)
    {
        ...
        {
            case RS_LEFT:
                p_next[0] += ( sin(phi+v) - sin(phi));
                p_next[1] += (-cos(phi+v) + cos(phi));
                p_next[2] = phi + v;
                break;
            case RS_RIGHT:
                p_next[0] += (-sin(phi-v) + sin(phi));
    ...
    p_next[0] = p_next[0] * rho_ + from[0];
    p_next[1] = p_next[1] * rho_ + from[1];
}
