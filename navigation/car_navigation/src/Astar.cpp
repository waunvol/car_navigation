...
astar::astar(nav_msgs::GetMap rec_map)
{
    map_msg = rec_map;
    ROS_INFO("Astar now start initing....");
...
bool astar::search()
{
    ...
    do  //搜索邻居最短点
    {
        ...
        while(1)
        {
            ...
            {
                min = tmp->weight + MHTdistance(tmp->way->x, tmp->way->y);      //记录最短距离(相比dijkstar多了权重优先)
